...
type NetworkReservation = Pick<
  GameState,
  "network"
>["network"]["reservations"][number];
    ...
    case "moving_to_collect": {
      ...
      if (
        ...
        return {
          ...
          taskGoal: resource ? `${resource} abholen` : undefined,
        };
      }
      ...
      if (
        ...
        return {
          ...
          taskGoal:
            reservation && job
              ? `${reservation.amount}× ${reservation.itemId} für ${job.recipeId}`
              : undefined,
        };
      }
      if (
        ...
        return {
          ...
          taskGoal: job
            ? `${job.output.count}× ${job.output.itemId} abholen`
            : undefined,
        };
      }
      const node = drone.targetNodeId
        ...
        : null;
      return {
        label: "Unterwegs zum Sammeln",
        taskGoal: node ? `${node.itemType} (${node.amount})` : undefined,
      };
    }
    case "collecting":
      if (drone.currentTaskType === "deconstruct") {
      ...
      if (drone.currentTaskType === "workbench_delivery")
        return { label: "Holt Werkbank-Output…" };
      if (drone.currentTaskType === "hub_dispatch")
    ...
    case "moving_to_dropoff":
      ...
      if (
        ...
        return {
          ...
          taskGoal: drone.cargo
            ? `${drone.cargo.amount}× ${drone.cargo.itemType}`
            : undefined,
        };
      }
      if (
        ...
        return {
          ...
          taskGoal: job
            ? `${job.output.count}× ${job.output.itemId}`
            : undefined,
        };
      }
      return {
