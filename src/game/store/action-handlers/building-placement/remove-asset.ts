...
function deriveDeliveredRefundForConstructionSite(input: {
  ...
  for (const [res, totalAmt] of Object.entries(totalCost)) {
    ...
    if (delivered > 0) {
      deliveredRefund[res as keyof Inventory] = Math.max(
        1,
        Math.floor(delivered / 3),
      );
    }
  }
...
export function handleRemoveAssetAction(
  ...
  if (bTypeR === "warehouse") {
    ...
    const restoredCellMap = minerState
      ? {
          ...removedB.cellMap,
          [cellKey(targetAsset.x, targetAsset.y)]: minerState.depositId,
        }
      : removedB.cellMap;
    partialRemove = {
      ...state,
  ...
  for (const sid of stripAssetIds) {
    if (partialRemove.constructionSites?.[sid]) {
      ...
      partialRemove = {
        ...partialRemove,
        constructionSites: restSites,
        inventory: invAfterSiteRefund,
      };
    }
  }
