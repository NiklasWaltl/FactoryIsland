...
import { getOrBuildRoutingIndex } from "../helpers/routing-index-cache";
import {
...
export function handleLogisticsTickAction(
  ...
  const conveyorRoutingIndex = getOrBuildRoutingIndex(state);
  ...
  const nextState = ctx.changed
    ...
    : state;
  return { ...nextState, routingIndexCache: conveyorRoutingIndex };
}
