...
import { hasResources } from "../../store/inventory-ops";
import {
    ...
    <div className="fi-build-items">
      ...
      >
        ...
        <div className="fi-build-item-info">
          ...
          <div className="fi-build-item-desc">
            Zeigt Stromknoten, Verbindungen, Verbraucher und Energie-Bilanz an.
          </div>
          <div
            className={`fi-build-status ${energyDebugOverlay ? "fi-build-status--ok" : "fi-build-status--no-res"}`}
          >
...
export const BuildMenu: React.FC<BuildMenuProps> = React.memo(
  ({ state, dispatch }) => {
    const selected = state.selectedBuildingType;
    const buildingInventoryView: Inventory =
    ...
    const canAfford = (bType: BuildingType): boolean =>
      hasResources(
        ...
        BUILDING_COSTS[bType] as Partial<Record<keyof Inventory, number>>,
      );

    const canAffordFloor = (tileType: FloorTileType): boolean =>
      hasResourcesInPhysicalStorage(
        state,
        FLOOR_TILE_COSTS[tileType] as Partial<Record<keyof Inventory, number>>,
      );

    const getBuildSourceDebugTitle = (
      costs: Partial<Record<keyof Inventory, number>>,
    ): string => {
      const missing = Object.entries(costs).flatMap(([res, amt]) => {
        const required = amt ?? 0;
        const available = (buildingInventoryView[res as keyof Inventory] ??
          0) as number;
        const shortfall = required - available;
        return shortfall > 0
          ? [`${shortfall} ${RESOURCE_LABELS[res] ?? res}`]
          : [];
      });
    ...
    const isAlreadyPlaced = (bType: BuildingType): boolean => {
      ...
      if (bType === "warehouse")
        return (
          state.warehousesPlaced >= (import.meta.env.DEV ? 100 : MAX_WAREHOUSES)
        );
    ...
    const getStatus = (
      ...
      if (isAlreadyPlaced(bType))
        return {
          ...
          className: "fi-build-status--placed",
        };
      if (!canAfford(bType))
        return {
          label: "Nicht genug Ressourcen",
          className: "fi-build-status--no-res",
        };
      return {
        label: "Kann platziert werden",
        className: "fi-build-status--ok",
      };
    };
    ...
    return (
      <div className="fi-build-menu" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fi-build-menu-hint">
          ...
          <br />
          Rechtsklick auf ein platziertes Gebäude zum Entfernen.
        </div>

        {BUILD_CATEGORIES.map((cat) => (
          <div key={cat.key} className="fi-build-category">
            <h3 className="fi-build-category-title">
              {cat.emoji} {cat.label}
            </h3>
            <div className="fi-build-items">
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      dispatch({
                        ...
                        buildingType: isSelected ? null : bType,
                      });
                    }}
                  >
                    <div className="fi-build-item-icon">
                      <img
                        src={ASSET_SPRITES[bType]}
                        alt={BUILDING_LABELS[bType]}
                        style={{
                          width: 36,
                          height: 36,
                    ...
                    <div className="fi-build-item-info">
                      <div className="fi-build-item-name">
                        ...
                        <span className="fi-build-item-size">
                          {size}×{size}
                        </span>
                      </div>
                      <div className="fi-build-item-desc">
                        {getBuildingDef(bType).description}
                      </div>
                      <div className="fi-build-item-costs">
                        {Object.entries(costs).map(([res, amt]) => {
                          const have = (buildingInventoryView[
                            res as keyof Inventory
                          ] ?? 0) as number;
                          const enough = have >= (amt ?? 0);
                          return (
                            <span
                              key={res}
                              className={`fi-build-cost ${enough ? "" : "fi-build-cost--lacking"}`}
                            >
                      ...
                      <div className={`fi-build-status ${status.className}`}>
                        {status.label}
                      </div>
                    </div>
                  </div>
                ...
                <div
                  ...
                  onClick={() =>
                    dispatch({
                      ...
                      tileType: isSelectedF ? null : tileType,
                    })
                  }
                >
                  <div className="fi-build-item-icon">
                    <img
                      src={
                        tileType === "stone_floor"
                          ? FLOOR_SPRITES.stone_floor
                          : GRASS_TILE_SPRITES[0]
                      }
                      alt={FLOOR_TILE_LABELS[tileType]}
                      style={{
                        width: 36,
                        height: 36,
                        imageRendering: "pixelated",
                  ...
                  <div className="fi-build-item-info">
                    ...
                    <div className="fi-build-item-desc">
                      {FLOOR_TILE_DESCRIPTIONS[tileType]}
                    </div>
                    <div className="fi-build-item-costs">
                      {Object.entries(costs).map(([res, amt]) => {
                        const have = (floorInventoryView[
                          res as keyof Inventory
                        ] ?? 0) as number;
                        const enough = have >= (amt ?? 0);
                        return (
                    ...
                    <div
                      className={`fi-build-status ${affordable ? "fi-build-status--ok" : "fi-build-status--no-res"}`}
                    >
                      {affordable
                        ? "Kann platziert werden"
