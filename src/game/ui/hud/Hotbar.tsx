...
export const Hotbar: React.FC<HotbarProps> = React.memo(
  ({ state, dispatch }) => {
    const handleDragStart = (
      e: React.DragEvent<HTMLButtonElement>,
      idx: number,
    ) => {
      if (state.hotbarSlots[idx].toolKind === "empty") {
        e.preventDefault();
        return;
      }
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>, _toIdx: number) => {
      ...
      if (source === "warehouse") {
        ...
        const bType = e.dataTransfer.getData("buildingType") as
          | BuildingType
          | "";
        ...
        {state.hotbarSlots.map((slot, idx) => {
          ...
          return (
            <div
              ...
              onDragOver={(e) => {
                e.preventDefault();
                e.currentTarget.classList.add("fi-hotbar-drop-zone--over");
              }}
              onDragLeave={(e) =>
                e.currentTarget.classList.remove("fi-hotbar-drop-zone--over")
              }
              onDrop={(e) => {
                e.stopPropagation();
                e.currentTarget.classList.remove("fi-hotbar-drop-zone--over");
            ...
            >
              <button
                ...
                onContextMenu={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  dispatch({ type: "REMOVE_FROM_HOTBAR", slot: idx });
                }}
                title={
                  slot.toolKind !== "empty"
                    ? `${slot.label} (Rechtsklick/Ziehen: ins Lager)`
                    : ""
                }
              >
                <span className="fi-hotbar-emoji">{slot.emoji}</span>
                {slot.amount > 1 && (
                  <span className="fi-hotbar-amount">{slot.amount}</span>
                )}
                <span className="fi-hotbar-label">
                  {slot.toolKind === "empty" ? "" : slot.label}
                </span>
