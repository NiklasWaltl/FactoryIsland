...
const NETWORK_ASSET_TYPES = new Set<AssetType>([
  "generator",
  "cable",
  "battery",
  "smithy",
...
export const PowerPolePanel: React.FC<PowerPolePanelProps> = React.memo(
  ({ state, dispatch }) => {
    ...
    const isConnected = poleId
      ? state.connectedAssetIds.includes(poleId)
      : false;
    ...
    const connectedInRange = inRange.filter((a) =>
      state.connectedAssetIds.includes(a.id),
    );
    const disconnectedInRange = inRange.filter(
      (a) => !state.connectedAssetIds.includes(a.id),
    );
    ...
    return (
      ...
      >
        ...
        {/* Connection status */}
        <div
          className={`fi-power-pole-status ${isConnected ? "fi-power-pole-status--connected" : "fi-power-pole-status--disconnected"}`}
        >
          ...
          {inRange.length === 0 ? (
            ...
            <ul className="fi-power-pole-asset-list">
              {connectedInRange.map((a) => (
                <li
                  key={a.id}
                  className="fi-power-pole-asset-item fi-power-pole-asset-item--connected"
                >
              ...
              {disconnectedInRange.map((a) => (
                <li
                  key={a.id}
                  className="fi-power-pole-asset-item fi-power-pole-asset-item--disconnected"
                >
                  <span>{ASSET_EMOJIS[a.type]}</span>
                  <span>{ASSET_LABELS[a.type]}</span>
