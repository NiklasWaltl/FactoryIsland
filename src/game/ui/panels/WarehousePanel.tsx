...
import { getZoneBuildingIds } from "../../store/selectors/zone-selectors";
...
export const WarehousePanel: React.FC<WarehousePanelProps> = React.memo(
  ({ state, dispatch }) => {
    const whCap = state.mode === "debug" ? Infinity : WAREHOUSE_CAPACITY;
    const selectedWarehouseId = state.selectedWarehouseId;
    const selectedWarehouseInv = selectedWarehouseId
      ? state.warehouseInventories[selectedWarehouseId]
      : null;
    ...
    const isWorkbenchTargetWarehouse = Object.values(state.assets).some(
      (asset) =>
        ...
        state.buildingSourceWarehouseIds[asset.id] === selectedWarehouseId,
    );
    const shouldRenderToolsSection =
      isWorkbenchTargetWarehouse || hasToolOrSeedStock;
    ...
    return (
      ...
      >
        ...
        {shouldRenderToolsSection ? (
          <>
            <h3 className="fi-panel-section-title fi-warehouse-tools-title">
              🔨 Werkzeuge
            </h3>
            {hasToolOrSeedStock ? (
              <>
                <p className="fi-warehouse-hint">
                  Nur diese Items können in die Hotbar gelegt werden.
                </p>
                <div
                  className="fi-warehouse-tools-list"
                  data-testid="wh-tools-list"
                >
                  {TOOL_ITEMS.map(({ key, kind }) => {
                    ...
                    return (
                      <div key={key} className="fi-warehouse-tool-row">
                        <span className="fi-warehouse-tool-emoji">
                          {RESOURCE_EMOJIS[key] ?? "?"}
                        </span>
                        <div className="fi-warehouse-tool-meta">
                          <span className="fi-warehouse-tool-name">
                            {RESOURCE_LABELS[key] ?? key}
                          </span>
                          <span className="fi-warehouse-tool-counts">
                            <span>
                              Im Lager: <strong>{amount}</strong>
                            </span>
                            <span className="fi-warehouse-tool-counts-sep">
                              ·
                            </span>
                            <span>
                              In Hotbar: <strong>{inHotbar}</strong>
                            </span>
                          </span>
                        </div>
                        <button
                          ...
                          disabled={!canEquip}
                          title={
                            canEquip ? "In die Hotbar legen" : "Nicht im Lager"
                          }
                          onClick={() =>
                            dispatch({
                  ...
                  {SEED_ITEMS.map(({ key, kind }) => {
                    ...
                    return (
                      <div
                        key={key}
                        className="fi-warehouse-tool-row fi-warehouse-tool-row--seed"
                      >
                        <span className="fi-warehouse-tool-emoji">
                          {RESOURCE_EMOJIS[key] ?? "?"}
                        </span>
              ...
              <div
                className="fi-warehouse-tools-empty"
                data-testid="wh-tools-empty-hint"
              >
                <p className="fi-warehouse-tools-empty-line">
                  Werkzeuge aus der Werkbank landen hier.
                </p>
        ...
        <h3 className="fi-panel-section-title fi-warehouse-materials-title">
          📦 Materialien
        </h3>
        <p className="fi-warehouse-hint fi-warehouse-hint--quiet">
          Automatisch eingelagert.
        </p>
        <ul
          className="fi-warehouse-materials-list"
          data-testid="wh-materials-list"
        >
          {MATERIAL_ITEMS.map((key) => {
            ...
            return (
              ...
              >
                <span className="fi-warehouse-material-emoji">
                  {RESOURCE_EMOJIS[key] ?? "?"}
                </span>
                <span className="fi-warehouse-material-name">
                  {RESOURCE_LABELS[key] ?? key}
                </span>
                <span className="fi-warehouse-material-amount">
                  {whAmount}
                  {whCap !== Infinity ? `/${whCap}` : ""}
                </span>
              </li>
            );
          })}
        ...
        {(() => {
          const currentZoneId = selectedWarehouseId
            ? (state.buildingZoneIds[selectedWarehouseId] ?? null)
            : null;
          const currentZone = currentZoneId
            ? (state.productionZones[currentZoneId] ?? null)
            : null;
              ...
              {currentZone ? (
                <div style={{ fontSize: 12, marginBottom: 4 }}>
                  <span style={{ color: "#7cb3f5" }}>
                    Zone: <strong>{currentZone.name}</strong>
                  </span>{" "}
                  ({getZoneWarehouseIds(state, currentZone.id).length}{" "}
                  Lagerh&auml;user,{" "}
                  {getZoneBuildingIds(state, currentZone.id).length}{" "}
                  Geb&auml;ude)
                  <button
                    ...
                    onClick={() =>
                      ...
                      dispatch({
                        ...
                        buildingId: selectedWarehouseId,
                        zoneId: null,
                      })
                    }
                  >
                  ...
                  <button
                    ...
                    style={{ marginLeft: 4 }}
                    onClick={() =>
                      dispatch({ type: "DELETE_ZONE", zoneId: currentZone.id })
                    }
                  >
                    Zone l&ouml;schen
                  </button>
                  ...
                  {(() => {
                    ...
                    const aggInv = getZoneAggregateInventory(
                      state,
                      currentZone.id,
                    );
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          {zWhIds.map((whId, i) => {
                            const idx =
                              Object.keys(state.warehouseInventories).indexOf(
                                whId,
                              ) + 1;
                            return (
                              <span key={whId}>
                        ...
                        {zWhIds.length > 0 && (
                          <div style={{ marginTop: 2 }}>
                            Zonenbestand (Kapazit&auml;t: {zoneCap}
                            /Ressource):&nbsp;
                            {(
                              [
                                "wood",
