...
import { MANUAL_ASSEMBLER_PROCESS_MS } from "../../store/constants/timing/workbench-timing";
...
export const ManualAssemblerPanel: React.FC<ManualAssemblerPanelProps> =
  React.memo(({ state, dispatch }) => {
    const hasAssembler = Object.values(state.assets).some(
    ...
    const progressPct = Math.max(
      0,
      Math.min(100, Math.round(assembler.progress * 100)),
    );
    const remainingMs = Math.max(
      0,
      Math.round((1 - assembler.progress) * MANUAL_ASSEMBLER_PROCESS_MS),
    );
      ...
      <div className="fi-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span>
              {RESOURCE_EMOJIS.ironIngot} {RESOURCE_LABELS.ironIngot}
            </span>
            <strong>{sourceInv.ironIngot}</strong>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span>
              {RESOURCE_EMOJIS.metalPlate} {RESOURCE_LABELS.metalPlate}
            </span>
            <strong>{sourceInv.metalPlate}</strong>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span>
              {RESOURCE_EMOJIS.gear} {RESOURCE_LABELS.gear}
            </span>
        ...
        <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
          ...
          <button
            ...
            onClick={() =>
              dispatch({ type: "MANUAL_ASSEMBLER_START", recipe: "gear" })
            }
          >
        ...
        <div style={{ marginTop: 12 }}>
          ...
          >
            ...
            <strong>
              {isBusy ? `${(remainingMs / 1000).toFixed(1)} s` : "Bereit"}
            </strong>
          </div>
          <div
            style={{
