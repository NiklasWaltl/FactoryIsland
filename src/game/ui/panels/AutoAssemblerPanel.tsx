...
export const AutoAssemblerPanel: React.FC<AutoAssemblerPanelProps> = React.memo(
  ({ state, dispatch }) => {
    ...
    const progressPct = entry.processing
      ? Math.max(
          ...
          Math.min(
            100,
            (entry.processing.progressMs / entry.processing.durationMs) * 100,
          ),
        )
      : 0;
        ...
        <div style={{ display: "grid", gap: 8 }}>
          ...
          >
            ...
            >
              <button
                className={
                  entry.selectedRecipe === "metal_plate"
                    ? "fi-btn fi-btn-sm--active"
                    : "fi-btn fi-btn-sm"
                }
                disabled={
                  !canChangeRecipe || entry.selectedRecipe === "metal_plate"
                }
                onClick={() =>
                  dispatch({
          ...
          {!canChangeRecipe ? (
            <p style={{ fontSize: 11, color: "#9ca3af", margin: 0 }}>
              Rezeptwechsel nur bei leerem Puffer, ohne laufende Produktion und
              ohne wartendes Output.
            </p>
          ) : null}
          ...
          <div style={{ fontSize: 12, color: "#cbd5e1" }}>
            ...
            {recipe ? (
              <div>
                Aktiv: {recipe.inputAmount}× Eisenbarren →{" "}
                {recipe.outputItem === "metalPlate"
                  ? "Metallplatte"
                  : "Zahnrad"}
              </div>
            ) : null}
          ...
          >
            <span>Strom</span>
            <span style={{ color: powerBadgeColor, fontWeight: 600 }}>
              {Math.round(powerPercent)}%
            </span>
          </div>
          <div style={{ fontSize: 11, color: "#9ca3af" }}>
            Leistung (nominal / effektiv): {currentEnergyPerSec.toFixed(1)} /{" "}
            {effectiveEnergyPerSec.toFixed(1)} J/s
          </div>
        </div>
      </div>
    );
