...
export const ConveyorSplitterPanel: React.FC<ConveyorSplitterPanelProps> = React.memo(({ state, dispatch }) => {
  ...
  if (!splitterId || !splitterAsset || splitterAsset.type !== "conveyor_splitter") {
    return null;
  }
    ...
    >
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {SIDE_CONFIG.map(({ side, label }) => {
          const currentFilter = getSplitterFilter(state.splitterFilterState, splitterId, side);
          return (
            <section
              key={side}
              style={{
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                <strong>{label}</strong>
                <span style={{ color: "#9ca3af", fontSize: 12 }}>{getFilterLabel(currentFilter)}</span>
              </div>
                ...
                {FILTER_OPTIONS.map((item) => {
                  ...
                  return (
                    <button
                      key={item ?? "all"}
                      className="fi-btn fi-btn-sm"
                      onClick={() => dispatch({ type: "SET_SPLITTER_FILTER", splitterId, side, itemType: item })}
                      style={{
                        minHeight: 32,
                        padding: "5px 8px",
                        border: selected ? "1px solid #7CFC00" : "1px solid rgba(255,255,255,0.16)",
                        background: selected ? "rgba(124,252,0,0.14)" : "rgba(255,255,255,0.05)",
                    ...
                    >
                      {item ? `${RESOURCE_EMOJIS[item] ?? ""} ${getFilterLabel(item)}` : "Alle"}
                    </button>
                  );
                })}
              </div>
