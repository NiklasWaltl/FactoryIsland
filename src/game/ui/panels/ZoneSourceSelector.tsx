 ...
 */
export const ZoneSourceSelector: React.FC<ZoneSourceSelectorProps> = React.memo(
  ({ state, buildingId, dispatch }) => {
    ...
    const currentZoneId = buildingId
      ...
      <>
        ...
        <div className="fi-workbench-source" style={{ marginTop: 4 }}>
          ...
          {zones.map((z) => {
            ...
            const isActive =
              info.source.kind === "zone" && info.source.zoneId === z.id;
        ...
        {info.isStale && (
          <div style={{ fontSize: 11, color: "#e8a946", marginTop: 2 }}>
            {"\u26A0"} Zugewiesenes Lagerhaus nicht mehr vorhanden {"\u2014"}{" "}
            Quelle: Global
          </div>
        )}
          ...
          >
            <div>
              ...
              {info.zoneWarehouseIds.map((whId, i) => {
                const idx =
                  Object.keys(state.warehouseInventories).indexOf(whId) + 1;
                return (
                  <span key={whId}>
                    {i > 0 ? ", " : ""}Lagerhaus {idx || "?"}
                  </span>
            ...
            {info.zoneBuildingIds.length > 0 && (
              <div>
                ...
                {info.zoneBuildingIds.map((bId, i) => {
                  ...
                  const label = asset
                    ? (BUILDING_LABELS[asset.type as BuildingType] ??
                      asset.type)
                    : bId;
                  return (
                    <span key={bId}>
        ...
        {info.fallbackReason === "zone_no_warehouses" && (
          <div
            data-testid="zone-warning"
            style={{ fontSize: 11, color: "#e8a946", marginTop: 2 }}
          >
            {"\u26A0"} Die zugewiesene Zone hat keine Lagerh{"\u00e4"}user{" "}
            {"\u2014"} weise zuerst ein Lagerhaus der Zone zu.
          </div>
        )}
              ...
              {JSON.stringify(
                {
                  ...
                  ...(info.source.kind === "zone"
                    ? {
                        aggregateInventory: getZoneAggregateInventory(
                          state,
                          info.source.zoneId,
                        ),
                        zoneCapacity: getZoneItemCapacity(
                          state,
                          info.source.zoneId,
                        ),
                      }
