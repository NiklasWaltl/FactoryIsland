...
export function migrateV0ToV1(raw: SaveGameV0): any {
  ...
  if (conveyorsRaw && typeof conveyorsRaw === "object") {
    for (const [id, value] of Object.entries(
      conveyorsRaw as Record<string, unknown>,
    )) {
          ...
          (acc, [id, smelter]) => {
            ...
            acc[id] = {
              inputBuffer: Array.isArray(s.inputBuffer)
                ...
                : [],
              processing:
                (s.processing as AutoSmelterEntry["processing"]) ?? null,
              ...
              status: (typeof s.status === "string"
                ? s.status
                : "IDLE") as AutoSmelterEntry["status"],
              lastRecipeInput: (typeof s.lastRecipeInput === "string"
                ? s.lastRecipeInput
                : null) as string | null,
              lastRecipeOutput: (typeof s.lastRecipeOutput === "string"
                ? s.lastRecipeOutput
                : null) as string | null,
              throughputEvents: Array.isArray(s.throughputEvents)
                ? (s.throughputEvents as number[])
                : [],
              selectedRecipe:
  ...
  return {
    ...
    purchasedBuildings: safeArray<BuildingType>(
      raw.purchasedBuildings,
      base.purchasedBuildings,
    ),
    placedBuildings: safeArray<BuildingType>(
      raw.placedBuildings,
      base.placedBuildings,
    ),
    warehousesPurchased: safeNum(
      raw.warehousesPurchased,
      base.warehousesPurchased,
    ),
    warehousesPlaced: safeNum(raw.warehousesPlaced, base.warehousesPlaced),
    warehouseInventories:
      Object.keys(migratedWhInvs).length > 0
        ? migratedWhInvs
        : safeRecord<Inventory>(
            raw.warehouseInventories,
    ...
    smithy:
      raw.smithy && typeof raw.smithy === "object"
    ...
    generator:
      raw.generator && typeof raw.generator === "object"
        ? {
            ...
            progress: 0,
            running: false,
            ...(raw.generator as Partial<GeneratorState>),
          }
    ...
    conveyors: {
      ...safeRecord<ConveyorState>(undefined, base.conveyors),
      ...normalizedConveyors,
    },
    autoSmelters,
    manualAssembler:
      raw.manualAssembler && typeof raw.manualAssembler === "object"
        ? {
            ...base.manualAssembler,
            ...(raw.manualAssembler as Partial<ManualAssemblerState>),
          }
        : base.manualAssembler,
    machinePowerRatio,
...
export function isRuntimeGameStateSnapshot(raw: unknown): raw is GameState {
  ...
  return (
    Array.isArray(candidate.connectedAssetIds) &&
