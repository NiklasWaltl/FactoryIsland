...
export function decideSourceInventoryPickupEligibility(input: {
  ...
  const { sourceExists, availableInSource, remainingNeed, carryCapacity } =
    input;

  if (!sourceExists) return { kind: "blocked", reason: "source_missing" };
  if (availableInSource <= 0)
    return { kind: "blocked", reason: "source_empty" };
  if (remainingNeed <= 0)
    return { kind: "blocked", reason: "no_remaining_need" };
...
export function decideCollectionNodePickupPlan(input: {
  ...
  if (taskType === "hub_restock" && hubId) {
    ...
    const remainingNeed = getConstructionRemainingNeed(
      deliveryTargetId,
      nodeItemType,
      droneId,
    );
    ...
    const remainingNeed = getBuildingRemainingNeed(
      deliveryTargetId,
      nodeItemType,
      droneId,
    );
