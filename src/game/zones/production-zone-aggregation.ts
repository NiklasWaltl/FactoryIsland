...
function createEmptyInventoryLike(
 ...
 */
export function getZoneWarehouseIds(
  state: GameState,
  zoneId: string,
): string[] {
  const result: string[] = [];
  for (const [bid, zid] of Object.entries(state.buildingZoneIds)) {
    ...
    if (
      state.assets[bid]?.type === "warehouse" &&
      state.warehouseInventories[bid]
    ) {
      result.push(bid);
    }
  }
 ...
 */
export function getZoneAggregateInventory(
  state: GameState,
  zoneId: string,
): Inventory {
